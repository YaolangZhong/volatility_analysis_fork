...
class ModelParams(NpzMixin):
    ...
    def check_consistency(self, mute: bool = False, tol: float = 1e-6) -> bool:
        ...
        pi_sum = np.sum(self.pi, axis=1)
        if not np.allclose(pi_sum, 1, atol=tol):
            logger.error("Check 6-1: ❌ For some (n,j), sum over exporters in pi is not 1.")
            inconsistent = True
        else:
            if not mute:
                logger.info("Check 6-1: ✅ For each (n,j), pi sums to 1.")
    ...
    def unpack(self):
        ...
        Useful for one‑liner assignments such as:
            (N, S, alpha, beta, gamma, theta, pi, tilde_tau,
             X, V, D, countries, sectors) = params.unpack()
        """
...
class ModelShocks(NpzMixin):
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        ...
        if getattr(self, "_initialized", False) and name in (
            "lambda_hat", "d_hat", "tilde_tau_prime"
        ):
            self.check_consistency(tol=1e-6, mute=CHECK_CONSISTENCY_MUTE)
    ...
    def check_consistency(self, tol: float = 1e-6, mute: bool = False) -> bool:
        ...
        try:
            ...
            assert self.d_hat.shape == (N, N, S), f"df_hat shape {self.d_hat.shape} != ({N}, {N}, {S})"
        ...
        # 3-1: All values > 0.
        if not np.all(self.d_hat > 0):
            invalid_positions = np.where(self.d_hat <= 0)
            logger.error("Check 3-1: ❌ Some d_hat values are <= 0 at positions %s. Values: %s",
                         invalid_positions, self.d_hat[invalid_positions])
            inconsistent = True
        else:
            if not mute:
                logger.info("Check 3-1: ✅ All d_hat values are > 0.")
        # 3-2: Diagonal elements must be ones.
        diag_d_hat = self.d_hat[np.arange(N), np.arange(N), :]
        if not np.allclose(diag_d_hat, 1.0, atol=tol):
            logger.error("Check 3-2: ❌ Diagonal elements of df_hat are not equal to 1. They are: %s", diag_d_hat)
            inconsistent = True
        else:
            if not mute:
                logger.info("Check 3-2: ✅ Diagonal elements of d_hat are 1.")
...
class ModelSol(NpzMixin):
    ...
    P_hat : (N, S) array (country, sector)
        Goods price change in sector s in each country i
    pi_hat : (N, N, S) array (importer, exporter, sector)
    ...
    P_hat:    np.ndarray  # shape (N, S)
    pi_hat:   np.ndarray  # shape (N, N, S)
    ...
    def unpack(self):
        ...
        Return a tuple of all init fields in their declared order.
        Example usage:
            (w_hat, c_hat, P_hat, pi_hat, X_prime, p_index, real_w, D_prime) = sol.unpack()
        """
        return tuple(getattr(self, f.name) for f in fields(self) if f.init)
