...
class PlotlyVisualizer:
    ...
    def create_bar_chart(self, 
        ...
        fig.update_traces(hovertemplate=f'{x_label}: %{{x}}<br>{y_label}: %{{y:.2f}}')
        fig.update_layout(
            xaxis_tickangle=-45,
...
class ModelVisualizationEngine:
    ...
    def visualize_comparison(self, sol1: ModelSol, sol2: ModelSol):
        ...
        if variable:
            ...
            if variable.endswith("_hat"):
                ...
                value = val2
                st.write(f"Variable shape: {np.shape(value)} (showing Model 2 values)")
            
            self._visualize_variable(value, variable)
    
    def _visualize_variable(self, value: np.ndarray, variable_name: str):
        """Internal method to handle visualization based on variable dimensions."""
        if isinstance(value, np.ndarray) and value.ndim == 1:
            self._visualize_1d_variable(value, variable_name)
        elif isinstance(value, np.ndarray) and value.ndim == 2:
            self._visualize_2d_variable(value, variable_name)
        elif isinstance(value, np.ndarray) and value.ndim == 3:
            self._visualize_3d_variable(value, variable_name)
    ...
    def _visualize_1d_variable(self, value: np.ndarray, variable_name: str):
        ...
        if selected_items:
            self.visualizer.visualize_1d_variable(
                value, variable_name, selected_items, fig_width, fig_height
            )
        else:
            st.info(f"No {label.lower()} selected.")
    
    def _visualize_2d_variable(self, value: np.ndarray, variable_name: str):
        ...
        if selected_countries and selected_sectors:
            ...
            self.visualizer.visualize_2d_variable(
                value, variable_name, selected_countries, selected_sectors, fig_width, fig_height
            )
        else:
            st.info("No countries or sectors selected.")
    
    def _visualize_3d_variable(self, value: np.ndarray, variable_name: str):
        ...
        if selected_importers and selected_exporters and selected_sectors:
            ...
            self.visualizer.visualize_3d_variable(
                ...
                selected_sectors, fig_width, fig_height
            )
        else:
